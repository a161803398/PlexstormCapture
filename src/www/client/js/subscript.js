...
function showDonate(userId, toShow, preMsg){
    donateInfo.innerHTML = subscriptTemplate[toShow].text.replace('{USER_ID}', '<span class="hightlight">' + escapeHtml(userId) + '</span>');
    curDonateSfx = new Audio();
    
    donateImg.src = ""; //reset donate image
    donateImg.src = "/popup/img/" + subscriptTemplate[toShow].img;
    curDonateSfx.src = "/popup/sound/" + subscriptTemplate[toShow].sound;
